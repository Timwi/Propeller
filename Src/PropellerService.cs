...
namespace RT.Propeller
{
    class PropellerService : SelfService
    {
        ...
        public bool IsRunning { get { return _isRunning && !_mainThread.Exited; } }
        ...
        private void doStop()
        {
            if (_mainThread != null)
            {
                _mainThread.ShouldExit = true;
                _mainThread.WaitExited();
                _mainThread = null;
            }
            _isRunning = false;
        }

        class MainThread : ThreadExiter
        {
            ...
            public MainThread()
            {
                _mainThread = new Thread(mainThreadFunction);
                _mainThread.Start();
            }

            private void mainThreadFunction()
            {
                _log = new ConsoleLogger { TimestampInUTC = true };
                ...
                ListeningThread currentListeningThread = null;
